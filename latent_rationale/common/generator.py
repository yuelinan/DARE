...
class IndependentGenerator_IB(nn.Module):
    """
    The Generator takes an input text and returns samples from p(z|x)
    """
    ...
    def __init__(self,
        ...
        super(IndependentGenerator_IB, self).__init__()
    ...
    def forward(self, x, mask,device):
        ...
        z_dist = self.z_layer(h)  # [B, T, 2]
        # print(z_dist.size())
        z_prob = F.softmax(z_dist,-1)
        # print(z_prob)
        self.z_prob = z_prob
        if self.training:  # sample
            # z = z_dist.sample()  # [B, T, 1]
            z = self.gumbel_softmax(z_dist,1,device)
            # z = F.gumbel_softmax(z_dist,hard=False,dim=2)
        else:  # deterministic
            # z = (z_dist.probs >= 0.5).float()   # [B, T, 1]
            z = (z_prob >= 0.5).float()
    ...
    def gumbel_softmax(self,input, temperature, device):
