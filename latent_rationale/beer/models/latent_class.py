...
class LatentRationaleModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed = nn.Embedding(339503, 200,padding_idx=1)
        self.classifier = Classifier(
            embed=self.embed, hidden_size=hidden_size, output_size=output_size,
            dropout=dropout, layer=layer)

        if self.dependent_z:
            self.latent_model = DependentLatentModel(
                embed=self.embed, hidden_size=hidden_size,
                dropout=dropout, layer=layer)
        else:
