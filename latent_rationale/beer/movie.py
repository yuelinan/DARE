...
from latent_rationale.beer.models.model_helpers2 import build_model
...
from latent_rationale.beer.util import \
    get_args, prepare_minibatch,prepare_minibatch_class, get_minibatch, \
    print_parameters, movie_reader, movie_annotations_reader, load_embeddings, \
    initialize_model_
from latent_rationale.beer.evaluate import \
    evaluate_rationale, get_examples, evaluate_loss,evaluate_dev_percent,evaluate_loss_class,evaluate_rationale_class
from latent_rationale.common.util import make_kv_string
import logging
from tqdm import tqdm
# 改logging，upper bound，shell
...
def train():
    ...
    train_data = list(movie_reader(
        cfg["train_path"], aspect=cfg["aspect"], max_len=cfg["max_len"]))
    dev_data = list(movie_reader(
        cfg["dev_path"], aspect=cfg["aspect"], max_len=cfg["max_len"]))
    test_data = movie_annotations_reader(cfg["test_path"], aspect=cfg["aspect"])
    ...
    # dataloader = DataLoader(train_data, batch_size = cfg["batch_size"], shuffle=True, num_workers=0, drop_last=False)
    # print(train_data[1:3])
    # main training loop
    for epoch in range(cfg['epochs']):  # when we run out of examples, shuffle and continue
        ...
        #for step,batch in enumerate(tqdm(dataloader)):
            ...
            x, targets, _ = prepare_minibatch_class(batch, Gmodel.vocab, device=device)
            ...
            # Gloss = Gloss + 0.01*upper_bound 0.0001*upper_bound
            Gloss = Gloss + 0.01*upper_bound
            optimizer.zero_grad()
            Gloss.backward()
        ...
        dev_eval = evaluate_loss_class(
            Gmodel, dev_data, batch_size=batch_size,
        ...
        if hasattr(Gmodel, "z"):
            ...
            test_precision, test_macro_prec, recall, macro_recall, percents = evaluate_rationale_class(
                Gmodel, test_data, aspect=aspect,
                device=device, path=path, batch_size=batch_size)
            
            percents_dev = evaluate_dev_percent(Gmodel, dev_data,device=device, path=path, batch_size=8)
